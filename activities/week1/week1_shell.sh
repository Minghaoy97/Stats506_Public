...
#
# Author(s): Aravind Mantravadi, Minghao Yang, Yiyi Xu, Erin Susan Cikanek
...
#< $new_file grep -n -E "DOEID|BRR" | cut -f 1 -d ":" | tr \\n ,


# <6> uncomment the next three lines and copy the one liner above
cols=$(< $new_file grep -n -E "DOEID|BRR" | cut -f 1 -d ":" | tr \\n ,)
...
cut -f $cols new_file> recs_brrweights.csv
head recs_brrweights.csv
